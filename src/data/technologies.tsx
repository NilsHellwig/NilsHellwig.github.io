export const TECHNOLOGIES = [
  {
    name: "Web Technologies (Frontend)",
    technologies: [
      "HTML",
      "CSS",
      "Tailwind",
      "shadCN",
      "JavaScript",
      "jQuery",
      "React (+Next.js)",
      "Vue.js (+Nuxt.js)",
      "Angular",
    ],
  },
  {
    name: "Web Technologies (Backend)",
    technologies: ["Node.js", "Django", "FastAPI", "socket.io"],
  },
  {
    name: "App Development",
    technologies: ["React Native (Expo)", "Native Android development", "Framework7"],
  },
  {
    name: "Data Analysis",
    technologies: ["pandas", "NumPy", "NLTK", "SpaCy", "Matplotlib", "Seaborn", "Plotly"],
  },
  {
    name: "Machine Learning",
    technologies: ["Scikit-learn", "Tensorflow / Keras", "Huggingface Transformer"],
  },
  {
    name: "Databases",
    technologies: ["Firebase", "mySQL", "MangoDB", "ElasticSearch", "Redis", "SQLite"],
  },
  {
    name: "CI/CD",
    technologies: ["GitHub Actions", "GitLab CI/CD", "Docker", "nginx", "Vercel"],
  },
  {
    name: "Testing",
    technologies: ["pytest", "Playwright"],
  },
  {
    name: "Versioning (git)",
    technologies: ["GitHub", "BitBucket", "GitLab"],
  },
  {
    name: "GUI-Prototyping",
    technologies: ["Adobe XD", "Figma"],
  },
  {
    name: "Image Editing",
    technologies: ["Affinity Photo", "GIMP", "Adobe Photoshop"],
  },
  {
    name: "Audio Editing",
    technologies: ["Audacity", "Logic Pro"],
  },
  {
    name: "Video Editing",
    technologies: ["iMovie", "Final Cut Pro"],
  },
];
